...
<div class="container">
    ...
    <div class="card shadow-lg border-0">
        <div class="card-body">
            ...
            <form action="{{ route('mata-kuliahs.update', $mataKuliah->kode_mk) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select name="kode_angkatan" class="form-select" required>
                        @foreach ($angkatans as $angkatan)
                            <option value="{{ $angkatan->kode_angkatan }}" {{ $angkatan->kode_angkatan == $mataKuliah->kode_angkatan ? 'selected' : '' }}>
                                {{ $angkatan->tahun }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="nip_dosen" class="form-select" required>
                        ...
                        <option value="{{ $dosen->nip }}" {{ $dosen->nip == $mataKuliah->nip_dosen ? 'selected' : '' }}>
                            {{ $dosen->nama_dosen }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="sks" class="form-control" value="{{ $mataKuliah->sks }}" min="1" required>
                </div>
