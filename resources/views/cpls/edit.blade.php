            ...
            <form action="{{ route('cpls.update', $cpl->kode_cpl) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="kode_cpl" id="kode_cpl" 
                           class="form-control" value="{{ $cpl->kode_cpl }}" readonly>
                </div>
                        ...
                        <select name="kode_prodi" id="kode_prodi" class="form-select">
                            @foreach($prodis as $prodi)
                                <option value="{{ $prodi->kode_prodi }}" 
                                    {{ $cpl->kode_prodi == $prodi->kode_prodi ? 'selected' : '' }}>
                    ...
                    <select name="kode_angkatan" id="kode_angkatan" class="form-select" required>
                        @foreach($angkatans as $angkatan)
                            <option value="{{ $angkatan->kode_angkatan }}" 
                                {{ $cpl->kode_angkatan == $angkatan->kode_angkatan ? 'selected' : '' }}>
                                {{ $angkatan->tahun }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="3" 
                              class="form-control" required>{{ $cpl->deskripsi }}</textarea>
                </div>

                <div class="d-flex justify-content-end">
                    <a href="{{ route('cpls.index') }}" class="btn btn-secondary me-2 px-4">Batal</a>
