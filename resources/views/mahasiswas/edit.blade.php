    ...
    <div class="container">
        ...
        <form action="{{ route('mahasiswas.update', $mahasiswa->nim) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select id="prodi" class="form-select" required>
                    <option value="">-- Pilih Prodi --</option>
                    @foreach($prodis as $prodi)
                        <option value="{{ $prodi->kode_prodi }}" {{ $mahasiswa->angkatan && $mahasiswa->angkatan->kode_prodi == $prodi->kode_prodi ? 'selected' : '' }}>
                            {{ $prodi->nama_prodi }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="kode_angkatan" id="kode_angkatan" class="form-select" required>
                    ...
                    @if($mahasiswa->angkatan)
                        <option value="{{ $mahasiswa->angkatan->kode_angkatan }}" selected>
